...
def evaluate_datasets(model, tokenizer, eval_datasets, results_dir, device="cuda"):
    ...
    plot_bar_chart({k: v["overall"] for k, v in base_results.items()}, 
                  os.path.join(results_dir, "baseline_chart.png"))
...
def plot_bar_chart(results, output_path):
    ...
    # 设置标题和标签
    plt.title('基础模型在各数据集上的准确率', fontsize=15)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="评估基础模型在各个数据集上的性能")
    ...
    parser.add_argument("--ceval_path", type=str, default="processed_data/ceval_dataset", 
                        help="C-Eval数据集路径")
    parser.add_argument("--cmmlu_path", type=str, default="processed_data/cmmlu_dataset", 
                        help="CMMLU数据集路径")
    parser.add_argument("--mmlu_path", type=str, default="processed_data/mmlu_dataset", 
                        help="MMLU数据集路径")
    parser.add_argument("--gsm8k_path", type=str, default="processed_data/gsm8k_dataset", 
                        help="GSM8K数据集路径（可选）")
    parser.add_argument("--hellaswag_path", type=str, default="processed_data/hellaswag_dataset", 
                        help="HellaSwag数据集路径（可选）")
    parser.add_argument("--results_dir", type=str, default="evaluation_results/baseline", 
                        help="结果保存目录")
