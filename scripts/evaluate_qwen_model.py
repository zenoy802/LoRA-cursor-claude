...
# -*- coding: utf-8 -*-
"""
评估Qwen模型在中文和英文数据集上的能力，分析灾难性遗忘情况
"""
...
def evaluate_datasets(base_model, lora_model, tokenizer, eval_datasets, results_dir, device="cuda"):
    ...
    with open(os.path.join(results_dir, "qwen_evaluation_results.json"), "w", encoding="utf-8") as f:
        json.dump(results, f, indent=2, ensure_ascii=False)
    
    # 绘制雷达图
    plot_radar_chart(changes, os.path.join(results_dir, "qwen_radar_chart.png"))
...
def plot_radar_chart(changes, output_path):
    ...
    plt.title('Qwen模型在不同数据集上的性能对比', size=15, y=1.1)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="评估Qwen模型在中文和英文数据集上的能力")
    parser.add_argument("--base_model_path", type=str, default="models/base_model", 
                        help="基础模型路径")
    parser.add_argument("--lora_model_path", type=str, default="results/lora-chinese-qwen", 
                        help="LoRA模型路径")
